...
export class RecruitmentDashboardComponent implements OnInit { kpis: any[] = [];
  ...
  ngOnInit() { this.buildSvgChart();
    ...
    this.http.get<any>(`${environment.apiUrl}/api/dashboard/metrics`).subscribe(metrics => { this.selectionRate   = metrics.selectionRate   || 0;
      ...
      this.kpis = [
        { label: 'Total Candidates', value: metrics.totalCandidates,        meta: 'All sources',                                       change: '+12%', up: true,  color: 'rgba(107,77,240,.15)',  icon: '👥' },
        { label: 'Total CVs',        value: metrics.totalCVs,               meta: `${metrics.interviewedEarlier || 0} interviewed`,    change: '+8%',  up: true,  color: 'rgba(59,189,234,.15)',  icon: '📋' },
        { label: 'Open Reqs',        value: metrics.totalJobs,              meta: 'Active demand',                                     change: '+2',   up: true,  color: 'rgba(232,145,42,.15)',  icon: '📌' },
        { label: 'Active Vendors',   value: metrics.activeVendors,          meta: `${metrics.totalVendors || 0} total`,                change: '—',    up: true,  color: 'rgba(22,163,74,.15)',   icon: '🏢' },
        { label: 'Selection Rate',   value: `${metrics.selectionRate || 0}%`, meta: 'Live',                                           change: '-2%',  up: false, color: 'rgba(220,38,38,.15)',   icon: '🎯' },
      ];

    ...
    ApplicationDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> GetDocuments(Guid entityId, [FromQuery] string entityType = "Employee")
        => Ok(await documentService.GetForEntityAsync(entityType, entityId, ""));
    ...
    {
        try { return Ok(await exitService.UpdateChecklistItemAsync(itemId, req.IsCompleted ? "Completed" : "Pending", "")); }
        catch (KeyNotFoundException ex) { return NotFound(new { message = ex.Message }); }
    }
}

...
namespace Decypher.Web.Services.AI
{
    ...
    public class MultiAgentOrchestratorService : IMultiAgentOrchestratorService
    {
        ...
        public MultiAgentOrchestratorService(
            ParsingAgentService parsing, MatchingAgentService matching,
            RankingAgentService ranking, ExplanationAgentService explanation,
            BiasDetectionAgentService bias, IAuditLogService audit,
            IConfiguration config, ILogger<MultiAgentOrchestratorService> log)
        {
            _parsing = parsing; _matching = matching; _ranking = ranking;
            _explanation = explanation; _bias = bias; _audit = audit;
            ...
            CancellationToken ct = default)
        {
            ...
            var explanation = Run("Explanation Agent",   4, () => _explanation.ProcessAsync(input, ct));
            var bias        = Run("Bias Detection Agent",5, () => _bias.ProcessAsync(input, ct));

            bool needsReview = ranking.Confidence < confidenceThreshold;

            foreach (var (agent, result) in new[]
            {
                (_parsing.AgentName, parsing), (_matching.AgentName, matching),
                (_ranking.AgentName, ranking), (_explanation.AgentName, explanation),
            ...
            {
                JobId = jobId,
                ParsingResult = parsing, MatchingResult = matching,
                RankingResult = ranking, ExplanationResult = explanation,
